...
const TAILWIND: DataTableClassNames = {
  ...
  search:
    "h-9 w-56 rounded-md border border-gray-300 px-3 text-sm outline-none placeholder:text-gray-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100",
  ...
  columnMenuItem:
    "flex items-center gap-1.5 rounded-md px-1 py-1 hover:bg-gray-50 data-[hidden]:opacity-60",
  columnMenuGrip:
    "flex h-7 w-5 cursor-grab items-center justify-center rounded text-gray-300 hover:text-gray-600 active:cursor-grabbing",
  columnMenuLabel: "flex-1 truncate text-sm text-gray-700",
  columnMenuVisibility:
    "flex h-7 w-7 items-center justify-center rounded-md text-gray-400 hover:bg-gray-100 hover:text-gray-700 data-[active]:text-gray-800",
  columnMenuPin:
    "flex h-7 w-7 items-center justify-center rounded-md text-gray-400 hover:bg-gray-100 data-[active]:bg-indigo-600 data-[active]:text-white",
  columnMenuReset:
    "mt-1 w-full rounded-md px-2 py-1.5 text-start text-sm font-medium text-indigo-600 hover:bg-indigo-50",
...
export function UnstyledDemo({
  ...
  locale,
  pageMode,
}: Readonly<{ mode: DataMode; locale: Locale; pageMode?: PageMode }>) {
  return (
    <UnstyledLike
      mode={mode}
      locale={locale}
