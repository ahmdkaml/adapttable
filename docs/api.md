...
| `GetCellSpan` / `BodyCell` / `CellSpanRequest` / `CellSpanAppearance`     | Span callback / one rendered body cell / `{ colSpan?, rowSpan? }` / `"merged" \| "plain"`. See [row and column spanning](./row-spanning.md).                       |
