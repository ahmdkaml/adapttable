 ...
 * shared `CLAUDE.md` "Filters and Columns overlay rules" for the Radix Themes
 ...
 *
 * Radix Themes' overlays are unusually faithful in jsdom: the popover restores
 * focus to its trigger on close (Radix runs its own `focusScope`, not a
 ...
 *  - the popover's content node carries `role="dialog"` but Radix Themes'
...
/** The Radix `Select` controls render as comboboxes named by their field label. */
...
async function openFilterForm(): Promise<HTMLElement> {
  ...
  // Radix's Popover/Dialog mount their content a tick after opening.
...
/**
 * Dismiss the overlay (Escape) and wait for Radix to unmount it, so the open
...
describe("filter overlay a11y (axe) — Radix", () => {
  ...
  // Radix detach its listeners cleanly.
  afterEach(cleanup);

  it("toggles aria-expanded on the popover trigger across open/close", async () => {
    renderTable();
    // Closed: the trigger advertises a collapsed disclosure. Radix's
    ...
    // Re-clicking the open trigger closes it (the toggle survives Radix's
  ...
  it("Escape closes the popover and restores focus to the trigger", async () => {
    renderTable();
    await openFilterForm();
    // Escape runs Radix's dismiss → onOpenChange(false) → setFiltersOpen(false).
    fireEvent.keyDown(document.body, { key: "Escape" });
    ...
    // Radix Themes' `Popover` runs its own focus scope, which jsdom honours:
  ...
  it("outside-click closes the popover", async () => {
    renderTable();
    await openFilterForm();
    // Radix's `DismissableLayer` listens for the full pointer→mouse→click
  ...
  it("opening the drawer moves focus into the dialog behind a backdrop", async () => {
    ...
    fireEvent.click(trigger());
    const dialog = await screen.findByRole("dialog");
    // Radix's Dialog is modal: focus moves into the dialog and the
    // `Dialog.Overlay` backdrop dims/blocks the background.
    await waitFor(() =>
      expect(dialog.contains(document.activeElement)).toBe(true)
    );
    expect(document.querySelector(".rt-DialogOverlay")).toBeTruthy();
    await closeOverlay();
  });
  ...
  it("Escape closes the drawer", async () => {
    ...
    // unmount. Radix detaches the dialog from the accessibility tree on close —
    // re-query the role rather than watch the captured node, which lingers
    // briefly during Radix's teardown.
    await waitFor(() => {
