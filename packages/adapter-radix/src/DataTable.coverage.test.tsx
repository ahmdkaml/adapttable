...
/** Open a Radix Select combobox and click one of its options. */
function pickOption(combobox: HTMLElement, optionLabel: string) {
  fireEvent.click(combobox);
  fireEvent.click(screen.getByRole("option", { name: optionLabel }));
}

describe("<DataTable> (Radix) coverage-fill", () => {
  ...
  it("opens and closes the filter drawer in drawer mode (Done button → onClose)", async () => {
    ...
    // Radix's drawer is a Dialog; the primary "Done" button calls onClose →
  ...
  it("closes the filter drawer on Escape (Radix dismiss → onClose)", async () => {
    renderHarness({
      override: { filters: <div>filter body</div>, filtersMode: "drawer" },
    });
    fireEvent.click(screen.getByRole("button", { name: /filters/i }));
