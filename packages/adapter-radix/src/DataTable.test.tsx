...
/**
 * Radix Themes' `Select` is a button-based combobox: it opens a portalled
...
describe("<DataTable> (Radix)", () => {
  ...
  it("closes the filter popover on Escape (Radix dismiss → onCloseFilters)", async () => {
    renderHarness({ override: { filters: <div>filter body</div> } });
    const trigger = screen.getByRole("button", { name: /filters/i });
    fireEvent.click(trigger);
    await screen.findByText("filter body");
    expect(trigger).toHaveAttribute("aria-expanded", "true");
    // Radix wires Escape to the document; the dismiss runs the popover's
...
describe("<DataTable> (Radix) density → table size", () => {
  // Radix Themes' `Table.Root` echoes our `size` onto its wrapping
  // `div.rt-TableRoot` as both `data-size` and an `rt-r-size-{n}` class — the
  ...
  function tableRoot(props: Parameters<typeof Harness>[0]): HTMLElement {
    ...
    return container.querySelector<HTMLElement>("[data-size]")!;
  }
  ...
  it('maps density="compact" to the smaller "1" size', () => {
    const root = tableRoot({ override: { density: "compact" } });
    expect(root.getAttribute("data-size")).toBe("1");
    expect(root.className).toContain("rt-r-size-1");
  });

  it('maps density="comfortable" to the larger "2" size', () => {
    const root = tableRoot({ override: { density: "comfortable" } });
    expect(root.getAttribute("data-size")).toBe("2");
    expect(root.className).toContain("rt-r-size-2");
  });
  ...
  it("lets an explicit size prop win over density", () => {
    const root = tableRoot({ override: { density: "compact", size: "3" } });
    expect(root.getAttribute("data-size")).toBe("3");
    expect(root.className).toContain("rt-r-size-3");
  });
});

describe("select-all-matching banner (Radix)", () => {
  const MANY: Row[] = [
    ...ROWS,
...
describe("the noResults slot (Radix)", () => {
