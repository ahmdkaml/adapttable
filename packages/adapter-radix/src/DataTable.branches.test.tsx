...
describe("chrome.tsx branches", () => {
  ...
  it("clears the sort when the sort select is emptied (value || undefined falsy)", () => {
    ...
    fireEvent.click(
      screen.getByRole("combobox", { name: defaultLabels.sortBy })
    );
    fireEvent.click(
      screen.getAllByRole("option", { name: defaultLabels.sortBy })[0]!
    );
...
describe("tables.tsx branches", () => {
  it("aligns an 'end' column to the end (justifyFor end branch)", () => {
    ...
    // Radix maps the cell `justify="end"` onto its right text-align token.
    const cell = screen.getByText("Alice").closest("td")!;
    expect(cell.className).toContain("rt-r-ta-right");
  });
  ...
  it("aligns a 'center' column (justifyFor center branch)", () => {
    ...
    const cell = screen.getByText("Alice").closest("td")!;
    expect(cell.className).toContain("rt-r-ta-center");
  });
