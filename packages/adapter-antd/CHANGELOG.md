...
- ff2c5ef: Group headers carry their `group-select` and `group-aggregate` part names, and
  a group's footer no longer renders a second selection checkbox for rows its
  header already controls.
- 2ac7bbd: Pinning a row no longer paints it over the next people when a cell span is in
...
- b356374: A virtualized table renders its body as the element antd draws there. antd's
...
- a93083f: Chakra filters, columns, and saved-views cards follow the host dark class after
  portalling. Ant Design filters stay under the trigger, 4px below it.
...
- 4738820: Antd range operators expose `filter-operator`. The base-ui header filter treats its listbox portal as nested, so picking an operator does not close the overlay.
...
- cc8667f: Column groups render each column once
