...
describe("export states (Ant Design)", () => {
  it("shows Ant Design's own loading affordance while the export runs", async () => {
    let settle!: () => void;
    ...
    // antd's own loading Button: it marks itself loading and shows its spinner
    // in the icon slot. It stays enabled — that is the kit's choice — and the
    // second click is refused by the handler rather than by the markup.
