...
describe("SavedViewsPanel", () => {
  ...
  it("honors the views* classes, so a preset styles it like the menu", () => {
    ...
    // The view's own name takes the menu's item class — same shape, a
    // full-width control captioned with the view. The five icon controls
    // beside it take the compact one.
    expect(screen.getAllByRole("button", { name: "Mine" })[0]).toHaveClass(
      "item-class"
    );
    for (const name of [
      "Rename view",
