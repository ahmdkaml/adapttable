...
/** The city (3rd data column) cell of every tbody row, in DOM order. */
...
describe("summaryRow (Chakra)", () => {
  it("aligns summary cells under their columns with empties for expansion/selection/actions and missing keys", () => {
    ...
    const cells = within(container.querySelector("tfoot")!).getAllByRole(
      "cell"
    );
  ...
  it("renders no tfoot without summaryRow", () => {
    const { container } = renderTable();
